...
struct THSaveData {
    ...
    //creatures_glitched: Option<Vec<String>>,
...
fn main() -> Result<()> {
    ...
    if let Some(glitched) = savedata.creatures_glitched {
        let needs: HashSet<Creature>    = HashSet::from_iter(Creature::achievement_list());
        let glitched: HashSet<Creature> = HashSet::from_iter(glitched);
        let remaining = needs.difference(&glitched);
